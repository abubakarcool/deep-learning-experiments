...
# Since our dataset contains text reviews, we need to clean and preprocess the text to prepare it for the n-grams model.
...
# Step 2: Convert Text into n-grams Representation
# Now, we need to transform the cleaned text into a numerical format using the CountVectorizer.
# 1. Convert cleaned text into a matrix of token counts.
# 2. Display the shape of the transformed data.
# 3. Check the vocabulary size.

vectorizer_ngram = CountVectorizer(ngram_range=(1, 2), max_features=500)  ## Use bigrams and trigrams in CountVectorizer=> 1-grams, 2-grams, and 3-grams
X = vectorizer_ngram.fit_transform(df['cleaned_review']) ## Transform text using BoW with N-grams

bow_df = pd.DataFrame(X.toarray(), columns=vectorizer_ngram.get_feature_names_out()) # Convert to DataFrame
print(f"BoW n-grams Matrix Shape: {bow_df.shape}")  # (50000, 500) # Display shape and first few rows
print(bow_df.head())
print(f"Vocabulary Size: {len(vectorizer_ngram.get_feature_names_out())}") # Display vocabulary size



# Step 3: Train Logistic Regression Model
# We will now use the N-Grams features to train a simple Logistic Regression model to classify reviews as positive or negative
